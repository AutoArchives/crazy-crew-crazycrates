...
public class Locale implements SettingsHolder {
    ...
    public static final Property<String> PRIZE_ERROR = newProperty("errors.prize-error", "{prefix}<red>An error has occurred in</red> <gold>{crate}</gold> <red>for</red> <gold>#{prize}. Contact your owner!</gold>");
    ...
    public static final Property<String> MUST_BE_CONSOLE_SENDER = newProperty("player.requirements.must-be-console-sender", "{prefix}<red>You must be using console to use this command.</red>");

    public static final Property<String> MUST_BE_LOOKING_AT_BLOCK = newProperty("player.requirements.must-be-looking-at-block", "{prefix}<red>You must be looking at a block.</red>");
