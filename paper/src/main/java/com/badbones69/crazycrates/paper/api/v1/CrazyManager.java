...
import com.badbones69.crazycrates.paper.api.v1.oldobjects.Tier;
...
import com.badbones69.crazycrates.paper.api.v1.events.PlayerReceiveKeyEvent.KeyReceiveReason;
...
import com.badbones69.crazycrates.paper.api.v2.utils.ItemUtils;
...
import com.badbones69.crazycrates.paper.api.v2.crates.types.quadcrates.CrateSchematic;
...
public class CrazyManager {
    ...
    private final HologramManager hologramManager = this.plugin.crazyManager().getHologramManager();
    ...
    public void loadCrates() {
        ...
        if (fileManager.isLogging()) CrazyLogger.info("Loading all crate information...");

        for (String crateName : fileManager.getAllCratesNames(this.plugin)) {
            try {
                ...
                if (crateType == CrateType.cosmic && tiers.isEmpty()) {
                    ...
                    CrazyLogger.warn("No tiers were found for this cosmic crate " + crateName + ".yml file.");
                    continue;
                }
                ...
                brokecrates.add(crateName);
                CrazyLogger.warn("There was an error while loading the " + crateName + ".yml file.");
                e.printStackTrace();
            }
        }
        ...
        if (fileManager.isLogging()) {
            CrazyLogger.info("All crate information has been loaded.");
            CrazyLogger.info("Loading all the physical crate locations.");
        }
            ...
            for (String locationName : locations.getConfigurationSection("Locations").getKeys(false)) {
                try {
                    ...
                    if (world != null && crate != null) {
                        ...
                        if (this.hologramManager != null) this.hologramManager.create(location, crate.getHologram());
        ...
        if (fileManager.isLogging()) {
            if (loadedAmount > 0 || brokeAmount > 0) {
                if (brokeAmount <= 0) {
                    CrazyLogger.info("All physical crate locations have been loaded.");
                } else {
                    CrazyLogger.info("Loaded " + loadedAmount + " physical crate locations.");
                    CrazyLogger.info("Failed to load " + brokeAmount + " physical crate locations.");
                }
            }
        }

        // Loading schematic files
        if (fileManager.isLogging()) CrazyLogger.info("Searching for schematics to load.");

        String[] schems = new File(this.plugin.getDataFolder() + "/schematics/").list();

        for (String schematicName : schems) {
            if (schematicName.endsWith(".nbt")) {
                crateSchematics.add(new CrateSchematic(schematicName.replace(".nbt", ""), new File(this.plugin.getDataFolder() + "/schematics/" + schematicName)));

                if (fileManager.isLogging()) CrazyLogger.info(schematicName + " was successfully found and loaded.");
            }
        }

        if (fileManager.isLogging()) CrazyLogger.info("All schematics were found and loaded.");
    ...
    public void cleanDataFile() {
        ...
        if (data.contains("Players")) {
            ...
            if (logging) CrazyLogger.info("Cleaning up the data.yml file.");
            ...
            if (removePlayers.size() > 0) {
                if (logging) CrazyLogger.info(removePlayers.size() + " player's data has been marked to be removed.");

                removePlayers.forEach(uuid -> data.set("Players." + uuid, null));

                if (logging) CrazyLogger.info("All empty player data has been removed.");
            }

            if (logging) CrazyLogger.info("The data.yml file has been cleaned.");
            Files.DATA.saveFile();
        }
    }
    ...
    public void openCrate(Player player, Crate crate, KeyType keyType, Location location, boolean virtualCrate, boolean checkHand) {
        ...
        switch (crate.getCrateType()) {
            //case MENU -> {
            //    boolean openMenu = this.plugin.getApiManager().getConfig().getProperty(ConfigSettings.ENABLE_CRATE_MENU);
            ...
            case fire_cracker -> {
                if (CrateControlListener.inUse.containsValue(location)) {
                    ...
                    if (virtualCrate) {
                        ...
                        FireCracker.startFireCracker(player, crate, keyType, location, this.hologramManager);
                    }
                }
            }
            case quick_crate -> {
                if (CrateControlListener.inUse.containsValue(location)) {
                    ...
                    if (virtualCrate && location.equals(player.getLocation())) {
                        ...
                        CrateControlListener.inUse.put(player, location);
                        QuickCrate.openCrate(player, location, crate, keyType, this.hologramManager);
                    }
                }
            }
        ...
        this.plugin.getEventLogger().logCrateEvent(player, crate, keyType, this.plugin.getApiManager().getConfig().getProperty(Config.LOG_CRATE_ACTIONS_FILE), this.plugin.getApiManager().getConfig().getProperty(Config.LOG_CRATE_ACTIONS_CONSOLE));
    }
    ...
    public void removeCrateLocation(String id) {
        ...
        if (location != null) {
            ...
            if (this.hologramManager != null) this.hologramManager.remove(location.getLocation());
        }
    }
    ...
    /**
     * Get a list of all the crates loaded into the this.plugin.
     *
    ...
    public Integer getQuadCrateTimer() {
        return this.plugin.getApiManager().getConfig().getProperty(Config.QUAD_CRATE_TIMER);
    }
    ...
    public Inventory loadPreview(Crate crate) {
        ...
        Inventory inv = this.plugin.getServer().createInventory(null, slots, Methods.sanitizeColor(file.getString("Crate.Name")));
    ...
    public void givePrize(Player player, Prize prize, Crate crate) {
        if (prize != null) {
            ...
            for (ItemBuilder item : prize.getItemBuilders()) {
                ...
                if (this.plugin.getApiManager().getPluginConfig().getProperty(PluginSupport.PLACEHOLDERAPI_SUPPORT)) {
            ...
            for (String command : prize.getCommands()) { // /give %player% iron %random%:1-64
                    ...
                    for (String word : cmd.split(" ")) {
                        if (word.startsWith("%random%:")) {
                            ...
                            try {
                                ...
                                CrazyLogger.warn("The prize " + prize.getName() + " in the " + prize.getCrate() + " crate has caused an error when trying to run a command.");
                                CrazyLogger.warn("Command: " + cmd);
                            }
                        } else {
                ...
                if (this.plugin.getApiManager().getPluginConfig().getProperty(PluginSupport.PLACEHOLDERAPI_SUPPORT)) command = PlaceholderAPI.setPlaceholders(player, command);
            ...
            for (String message : prize.getMessages()) {
                if (this.plugin.getApiManager().getPluginConfig().getProperty(PluginSupport.PLACEHOLDERAPI_SUPPORT)) message = PlaceholderAPI.setPlaceholders(player, message);
            ...
            CrazyLogger.warn("No prize was found when giving " + player.getName() + " a prize.");
        }
    }
    ...
    public void addKeys(int amount, Player player, Crate crate, KeyType keyType) {
        switch (keyType) {
            case PHYSICAL_KEY -> {
                if (Methods.isInventoryFull(player)) {
                    if (this.plugin.getApiManager().getConfig().getProperty(Config.GIVE_VIRTUAL_KEYS_WHEN_INVENTORY_FULL)) {
                        addVirtualKeys(amount, player, crate);

                        if (this.plugin.getApiManager().getConfig().getProperty(Config.GIVE_VIRTUAL_KEYS_WHEN_INVENTORY_FULL_MESSAGE)) player.sendMessage(Messages.CANNOT_GIVE_PLAYER_KEYS.getMessage().replaceAll("%amount%", String.valueOf(amount)).replaceAll("%key%", crate.getName()));
                    } else {
