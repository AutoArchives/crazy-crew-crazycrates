...
import com.badbones69.crazycrates.folia.api.events.PhysicalCrateKeyCheckEvent;
import com.badbones69.crazycrates.folia.api.objects.CrateLocation;
import com.badbones69.crazycrates.folia.utils.Methods;
import com.badbones69.crazycrates.folia.api.CrazyManager;
import com.badbones69.crazycrates.folia.api.FileManager.Files;
import com.badbones69.crazycrates.folia.api.objects.Crate;
import com.badbones69.crazycrates.folia.cratetypes.QuickCrate;
...
public class CrateControlListener implements Listener { // Crate Control
    ...
    public void onCrateOpen(PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.LEFT_CLICK_BLOCK) {
            ...
            for (com.badbones69.crazycrates.folia.api.objects.CrateLocation loc : crazyManager.getCrateLocations()) {
                ...
                if (loc.getLocation().equals(clickedBlock.getLocation())) {
                    ...
                    if (player.getGameMode() == GameMode.CREATIVE && player.isSneaking() && player.hasPermission("crazycrates.admin")) {
                        ...
                        player.sendMessage(com.badbones69.crazycrates.folia.api.enums.settings.Messages.REMOVED_PHYSICAL_CRATE.getMessage("%ID%", loc.getID()));
                        return;
                    }
                    ...
                    if (loc.getCrateType() != CrateType.MENU) {
                        if (loc.getCrate().isPreviewEnabled()) {
                            com.badbones69.crazycrates.folia.listeners.PreviewListener.setPlayerInMenu(player, false);
            ...
            if (crateLocation != null && crateLocation.getCrate() != null) {
                ...
                com.badbones69.crazycrates.folia.api.events.PhysicalCrateKeyCheckEvent event = new PhysicalCrateKeyCheckEvent(player, crateLocation);
                ...
                if (!event.isCancelled()) {
                    ...
                    if (requiredKeys > 0 && totalKeys < requiredKeys) {
                        player.sendMessage(com.badbones69.crazycrates.folia.api.enums.settings.Messages.REQUIRED_KEYS.getMessage().replaceAll("%key-amount%", String.valueOf(requiredKeys)).replaceAll("%crate%", crate.getPreviewName()).replaceAll("%amount%", String.valueOf(totalKeys)));
                        return;
                    }
                    ...
                    if (hasKey) {
                        ...
                        if (!useQuickCrateAgain) {
                            if (crazyManager.isInOpeningList(player)) {
                                player.sendMessage(com.badbones69.crazycrates.folia.api.enums.settings.Messages.ALREADY_OPENING_CRATE.getMessage("%Key%", keyName));
                                return;
                            }
                        ...
                        if (com.badbones69.crazycrates.folia.utils.Methods.isInventoryFull(player)) {
                            player.sendMessage(com.badbones69.crazycrates.folia.api.enums.settings.Messages.INVENTORY_FULL.getMessage());
                            return;
                        }
    ...
    public void onAdminMenuClick(InventoryClickEvent e) {
        ...
        if (inv != null && e.getView().getTitle().equals(com.badbones69.crazycrates.folia.utils.Methods.sanitizeColor("&4&lAdmin Keys"))) {
            e.setCancelled(true);

            if (!com.badbones69.crazycrates.folia.utils.Methods.permCheck(player, Permissions.ADMIN_ACCESS, false)) {
            ...
            if (e.getRawSlot() < inv.getSize() && e.getRawSlot() >= 0) { // Clicked in the admin menu.
                ...
                if (crazyManager.isKey(item)) {
                    ...
                    if (e.getAction() == InventoryAction.PICKUP_ALL) {
                        ...
                        player.sendMessage(com.badbones69.crazycrates.folia.utils.Methods.getPrefix() + Methods.color("&a&l+1 " + (name != null ? name : crate.getName())));
                    }
                }
            }
        }
