package com.badbones69.crazycrates.folia.api.enums.settings;
...
public enum Messages {
    ...
    public static String convertList(List<String> list) {
        ...
        for (String line : list) {
            message.append(com.badbones69.crazycrates.folia.utils.Methods.color(line)).append("\n");
        }
    ...
    private String getMessage(Map<String, String> placeholders, boolean prefix) {
        ...
        if (isList) {
            if (exists) {
                message = com.badbones69.crazycrates.folia.utils.Methods.color(convertList(FileManager.Files.MESSAGES.getFile().getStringList("Messages." + path)));
            } else {
                message = com.badbones69.crazycrates.folia.utils.Methods.color(convertList(getDefaultListMessage()));
            }
        } else {
            if (exists) {
                message = com.badbones69.crazycrates.folia.utils.Methods.color(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
                message = com.badbones69.crazycrates.folia.utils.Methods.color(getDefaultMessage());
            }
        }
        ...
        if (isList) { // Don't want to add a prefix to a list of messages.
            return com.badbones69.crazycrates.folia.utils.Methods.color(message);
        } else { // If the message isn't a list.
            if (prefix) { // If the message needs a prefix.
                return com.badbones69.crazycrates.folia.utils.Methods.getPrefix(message);
